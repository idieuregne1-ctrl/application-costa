  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside
        ...
        aria-modal="true"
        aria-label="Filtres"
        className="relative flex h-full w-full max-w-sm flex-col bg-slate-900 shadow-2xl"
      >
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-4">
          ...
          <Group label="">
            <label className="flex cursor-pointer items-center justify-between">
              <span className="text-sm text-slate-200">Ouvert maintenant</span>
              <Switch
          ...
          <Group label="Sources actives">
              ...
              {SOURCE_OPTIONS.map((src) => (
                <label key={src.value} className="flex cursor-pointer items-center justify-between">
                  <span className="text-sm text-slate-200">
                    {src.label}
                    {src.note && <span className="ml-1 text-[11px] text-slate-500">({src.note})</span>}
                  </span>
                  <Switch
            ...
            <p className="mt-1 text-[11px] text-slate-500">Catégorie active : {activeCategory}</p>
          </Group>
        </div>

        <footer className="flex gap-2 border-t border-slate-800 p-4">
          <button
            onClick={reset}
            className="flex-1 rounded-xl border border-slate-700 py-3 text-sm font-medium text-slate-300 hover:bg-slate-800"
          >
            Réinitialiser
          </button>
          <button
            onClick={apply}
            className="flex-[2] rounded-xl bg-creme-500 py-3 text-sm font-semibold text-slate-950 hover:bg-creme-400"
          >
            Appliquer
          </button>
        </footer>
...
function Group({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      {label && <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</h3>}
      {children}
    </div>
  )
}
  ...
  return (
    <button
      ...
      className={[
        ...
        active
          ? 'border-creme-500 bg-creme-500/15 text-creme-300'
  ...
  return (
    <button
      ...
      className={[
        ...
        checked ? 'bg-creme-500' : 'bg-slate-700',
      ].join(' ')}
    >
      <span
        className={[

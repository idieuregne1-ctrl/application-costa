  ...
  return (
    <section className="rounded-xl border border-line bg-white p-3.5">
      ...
      {isLoadingCache ? (
        ...
        <p className="text-sm text-stone-400">{t("Pas assez d'avis pour générer un résumé fiable.")}</p>
      ) : isGenerating ? (
        ...
        <div className="space-y-2">
          ...
          <GenerateButton onClick={() => generate()} label={t('Résumer les avis')} />
        </div>
      ) : (
        <div className="space-y-2">
          <p className="text-xs text-stone-500">{t("Le verdict en un coup d'œil — sans lire 30 commentaires.")}</p>
          <GenerateButton onClick={() => generate()} label={t('Résumer les avis')} />
        </div>
      )}
    </section>
...
function SummaryView({ summary }: { summary: ReviewSummary }) {
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      {summary.loves.length > 0 && <List title={t('On aime')} items={summary.loves} accent="emerald" />}
      {summary.caveats.length > 0 && <List title={t('Bémols')} items={summary.caveats} accent="amber" />}
      {summary.bestFor.length > 0 && (
        <div>
          <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-stone-400">{t('Idéal pour')}</p>

...
export default function DownloadedZonesSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
 ...
 return (
 <div key={zone.id} className="rounded-xl border border-line bg-stone-50 p-3">
 <div className="flex items-start justify-between gap-2">
 <div className="min-w-0">
 <p className="truncate font-medium text-ink">{zone.label}</p>
 ...
 className="flex-1 rounded-lg border border-line py-1.5 text-xs font-medium text-stone-600 hover:bg-stone-100 disabled:opacity-40"
 >
 {refreshing === zone.id ? 'Rafraîchissement…' : 'Rafraîchir'}
 </button>
 <button
 onClick={() => remove(zone.id)}
 className="flex-1 rounded-lg border border-line py-1.5 text-xs font-medium text-red-700 hover:bg-stone-100"
 >
 Supprimer
 </button>
 </div>
